import { URL, skipSlashes } from '../platform/URL';
...
export function getRepoPaths(manager: PackageManager, basePkgName: string | false, baseKey: string) {
	...
	const resultPreferred: { isPreferred?: boolean, root: string, cdn?: CDN }[] = [];
	const resultOther: { isPreferred?: boolean, root: string, cdn?: CDN }[] = [];
	...
	for(let cdn of manager.cdnList) {
		resultOther.push({ isPreferred: true, root: cdn.root, cdn });
	}
...
export function parsePackage(
	...
	for(let field of manager.mainFields) {
